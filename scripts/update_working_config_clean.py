...
def main():
    ...
    if current_exec_match:
        ...
        current_content = re.sub(exec_pattern, new_exec_section, current_content, flags=re.DOTALL)
    ...
    if current_role_match:
        ...
        current_content = re.sub(role_pattern, new_role_section, current_content, flags=re.DOTALL)
    ...
    if current_alias_match:
        alias_list_str = ',\n        '.join(f'"{alias}"' for alias in company_aliases_list)
        new_alias_section = f'"company_aliases": [\n        {alias_list_str}\n    ]'
        current_content = re.sub(alias_pattern, new_alias_section, current_content, flags=re.DOTALL)
    ...
    print(f"  - {len(executive_names_list)} executive names (only normalized_name, no special chars)")
    print(f"  - {len(role_keywords_list)} role keywords (only role_canon)")
    print(f"  - {len(company_aliases_list)} company aliases (company names + tickers, cleaned)")
