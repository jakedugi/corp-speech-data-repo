#!/usr/bin/env python3
"""
Fix the corrupted working_config.py by recreating it cleanly with proper list formatting.
"""
...
def format_python_list(items, max_line_length=80):
    """Format a list of items as a properly formatted Python list string."""
    if not items:
        return "[]"
    ...
    for item in items:
        ...
        if current_line:
            ...
            test_line = current_line + ", " + item_str
...
def main():
    ...
    for filename in roles_files:
        ...
        if filepath.exists():
            with open(filepath, "r", newline="", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                for row in reader:
                    role = clean_role_canon(
                        row.get("role_raw", "")
                    )  # Use role_raw for more variety
                    if role:
    ...
    # Create the clean config with properly formatted lists
...
config = {{
    "nlp": {{
        ...
        "enable_ner": True,
        "use_gpu": False,
        "role_keywords": {format_python_list(role_keywords_list)},
        "executive_names": {format_python_list(executive_names_list)}
    }},
    "extraction": {{
        "company_aliases": {format_python_list(company_aliases_list)},
...
"""
    ...
    print(
        f"   - {len(executive_names_list)} executive names (normalized, no special chars)"
    )
    print(f"   - {len(role_keywords_list)} role keywords (canonical)")
