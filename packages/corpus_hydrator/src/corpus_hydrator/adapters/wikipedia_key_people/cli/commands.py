...
def scrape_index(
    index: str = typer.Option(
        "sp500", "--index", "-i", help="Market index to scrape (sp500, dow, nasdaq100)"
    ),
    output_dir: Path = typer.Option(
        Path("data"), "--output-dir", "-o", help="Output directory for scraped data"
    ),
    max_companies: Optional[int] = typer.Option(
        None, "--max-companies", "-m", help="Maximum companies to scrape (default: all)"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Run without making HTTP requests (for testing)"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Enable verbose logging"
    ),
    ...
    try:
        ...
        for company in result.companies:
            if company.key_people_count > 0:
                company_counts[f"{company.ticker} ({company.company_name})"] = (
                    company.key_people_count
                )
...
def scrape_multiple(
    indices: List[str] = typer.Option(
        ["sp500"], "--indices", "-i", help="Market indices to scrape"
    ),
    output_dir: Path = typer.Option(
        Path("data"), "--output-dir", "-o", help="Output directory for scraped data"
    ),
    max_companies: Optional[int] = typer.Option(
        None, "--max-companies", "-m", help="Maximum companies to scrape per index"
    ),
    dry_run: bool = typer.Option(
    ...
    try:
        ...
        for index_name, result in results.items():
            if result.success:
                ...
                print(
                    f"{index_name}: {result.companies_successful}/{result.companies_processed} companies, {result.total_key_people} people"
                )
            else:
...
def _save_results(result: WikipediaExtractionResult, output_dir: Path):
    ...
    with open(stats_file, "w") as f:
        f.write(
            f"=== {result.index_name.upper()} Key People Extraction Statistics ===\n\n"
        )
