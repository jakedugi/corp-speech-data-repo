...
class WikipediaKeyPerson(BaseModel):
    ...
    wikipedia_url: str = Field(..., description="Wikipedia page URL where data was found")
    extraction_method: str = Field(..., description="Method used for extraction (infobox, section, etc.)")
    scraped_at: datetime = Field(default_factory=datetime.now, description="When this data was scraped")

    # Quality indicators
    parse_success: bool = Field(default=True, description="Whether parsing was successful")
    confidence_score: float = Field(default=1.0, description="Confidence in data accuracy (0-1)")

    @validator("ticker")
    def validate_ticker(cls, v: str) -> str:
        """Validate ticker format."""
        if not re.match(r"^[A-Z]{1,5}(\.[A-Z])?$", v.upper()):
            raise ValueError("Ticker must be 1-5 uppercase letters optionally followed by . and letter")
        return v.upper()
...
class WikipediaCompany(BaseModel):
    ...
    index_name: str = Field(..., description="Index this company belongs to (sp500, dow, etc.)")

    # Processing metadata
    processed_at: datetime = Field(default_factory=datetime.now, description="When this company was processed")
...
class WikipediaExtractionResult(BaseModel):
    ...
    # Operation metadata
    operation_id: str = Field(..., description="Unique identifier for this extraction operation")
    index_name: str = Field(..., description="Index that was processed")
    started_at: datetime = Field(default_factory=datetime.now, description="When extraction started")
    completed_at: Optional[datetime] = Field(None, description="When extraction completed")

    # Results
    companies_processed: int = Field(default=0, description="Number of companies processed")
    companies_successful: int = Field(default=0, description="Number of companies successfully processed")
    ...
    # Data
    companies: List[WikipediaCompany] = Field(default_factory=list, description="Processed companies")
    key_people: List[WikipediaKeyPerson] = Field(default_factory=list, description="Extracted key people")
...
class WikipediaContentConfig(BaseModel):
    ...
    role_keywords: List[str] = Field(
        default_factory=lambda: [
            ...
            "officer", "team", "executives", "board members",
            "directors", "corporate officers", "leadership team",
            "management team", "senior management", "chairman",
            "chairperson", "president", "vice president",
            "chief executive", "chief operating", "chief financial",
            "chief technology", "chief information", "chief legal",
            "chief marketing", "chief human resources", "chief strategy",
            "founder", "co-founder", "founders", "owners",
            "principal", "principals", "partners", "managing director",
...
def validate_key_person(person: WikipediaKeyPerson) -> List[str]:
    ...
    # Check data quality
    if len(person.clean_name.split()) < 2 and not any(title in person.clean_title.lower() for title in ['chairman', 'president', 'founder']):
...
class NormalizedCompany(BaseModel):
    ...
    company_id: str = Field(..., description="Unique stable identifier (ticker + index)")
    company_name: str = Field(..., description="Full company name")
    ticker: str = Field(..., description="Stock ticker symbol")
    wikipedia_url: str = Field(..., description="Wikipedia company page URL")
    wikidata_qid: Optional[str] = Field(None, description="Wikidata QID (Q followed by digits)")
    index_name: str = Field(..., description="Index this company belongs to")
    processed_at: datetime = Field(default_factory=datetime.now, description="When this record was processed")
...
class NormalizedPerson(BaseModel):
    ...
    wikidata_qid: Optional[str] = Field(None, description="Wikidata QID for this person")
...
NORMALIZED_ROLE_VOCABULARY = {
    "CEO", "CFO", "COO", "CTO", "CIO", "CMO", "CRO", "CCO", "CSO", "CGO",
    "CHAIR", "VICE_CHAIR", "PRESIDENT", "VICE_PRESIDENT",
    "FOUNDER", "CO_FOUNDER", "EXECUTIVE",
    "BOARD_MEMBER", "BOARD_CHAIR", "BOARD_VICE_CHAIR",
    "EXECUTIVE_CHAIRMAN", "NON_EXECUTIVE_CHAIRMAN",
    "SENIOR_VICE_PRESIDENT", "VICE_PRESIDENT",
    "GENERAL_COUNSEL", "SECRETARY", "TREASURER",
    "CHIEF_EXECUTIVE_OFFICER", "CHIEF_FINANCIAL_OFFICER",
    "CHIEF_OPERATING_OFFICER", "CHIEF_TECHNOLOGY_OFFICER",
    "CHIEF_INFORMATION_OFFICER", "CHIEF_MARKETING_OFFICER",
    "CHIEF_RISK_OFFICER", "CHIEF_COMPLIANCE_OFFICER",
    "CHIEF_STRATEGY_OFFICER", "CHIEF_GROWTH_OFFICER"
}
...
class NormalizedRole(BaseModel):
    ...
    role_canon: str = Field(..., description="Canonical role from controlled vocabulary")
...
class NormalizedAppointment(BaseModel):
    ...
    confidence_score: float = Field(default=1.0, ge=0.0, le=1.0, description="Confidence in this appointment")
...
class DatasetManifest(BaseModel):
    ...
    extracted_at: datetime = Field(default_factory=datetime.now, description="When extraction completed")
    ...
    appointments_sha256: Optional[str] = Field(None, description="SHA256 of appointments.csv")

    # Processing metadata
    revision_ids: Dict[str, str] = Field(default_factory=dict, description="Source revision IDs")
    ...
    # Governance
    license_notice: str = Field(default="Wikipedia content available under CC BY-SA 3.0. Wikidata content available under CC0.", description="License attribution")
