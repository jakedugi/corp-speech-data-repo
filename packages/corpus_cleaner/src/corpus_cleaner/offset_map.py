...
def map_raw_to_norm(
    raw_text: str, norm_text: str, transformations: List[Dict[str, Any]]
...
def apply_offset_mapping(
    original_spans: List[Tuple[int, int]], offset_map: List[Tuple[int, int, int, int]]
) -> List[Tuple[int, int]]:
    """
...
def validate_offset_map(
    raw_text: str, norm_text: str, offset_map: List[Tuple[int, int, int, int]]
) -> bool:
    """
    ...
    for raw_start, raw_end, norm_start, norm_end in offset_map:
        if (
            ...
            or norm_start < 0
            or norm_end > len(norm_text)
        ):
            logger.warning(
                f"Invalid offset mapping: raw[{raw_start}:{raw_end}] -> norm[{norm_start}:{norm_end}]"
            )
            return False
