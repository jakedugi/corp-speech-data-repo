...
class TestMinFeatures:
    ...
    def test_passes_feature_filter(self):
        ...
        assert passes_feature_filter(context_both, 0)
        assert passes_feature_filter(context_one, 0)
        assert passes_feature_filter(context_none, 0)

        # min_features = 1
        assert passes_feature_filter(context_both, 1)
        assert passes_feature_filter(context_one, 1)
        assert not passes_feature_filter(context_none, 1)

        # min_features = 2
        assert passes_feature_filter(context_both, 2)
        assert passes_feature_filter(context_one, 2)  # "Settlement amount" = 2 votes
        assert not passes_feature_filter(context_none, 2)
...
class TestCourtNameExtraction:
    ...
    def test_is_bankruptcy_court(self):
        ...
        assert is_bankruptcy_court(bankruptcy_text)

        # Test district court (not bankruptcy)
        district_text = (
            "IN THE UNITED STATES DISTRICT COURT FOR THE SOUTHERN DISTRICT OF FLORIDA"
        )
        assert not is_bankruptcy_court(district_text)

        # Test court of appeals (not bankruptcy)
        appeals_text = "UNITED STATES COURT OF APPEALS FOR THE ELEVENTH CIRCUIT"
        assert not is_bankruptcy_court(appeals_text)

        # Test no court name found
        no_court_text = "This is just some random text without a court name."
        assert not is_bankruptcy_court(no_court_text)
...
class TestTitleDetection:
    ...
    def test_passes_enhanced_feature_filter_with_titles(self):
        ...
        assert passes_enhanced_feature_filter_with_titles(
