...
from ..extraction_pipeline.extract_cash_amounts_stage1 import (
    AMOUNT_REGEX,
)
from ..extraction_pipeline.extract_cash_amounts_stage1 import (
                ...
                try:
                    ...
                    if not disable_regex:
                        ...
                        for m in regex_matches:
                            ...
                            out.append(
                                CashAmountCandidate(val, amt, ctx, feature_votes)
                            )
...
def rewrite_stage_file(
    ...
    with (
        input_file.open(encoding="utf8") as fin,
        tmp.open("w", encoding="utf8") as fout,
    ):
        for line in fin:
            rec = json.loads(line)
