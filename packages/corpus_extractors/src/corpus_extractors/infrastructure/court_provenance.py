...
class CourtProvenanceExtractor:
    ...
    def extract_provenance_from_quote(self, quote: Dict[str, any]) -> Dict[str, any]:
        ...
        result["court"] = (
            self.extract_court_from_doc_id(doc_id)
            or self.extract_court_from_path(src_path)
            or self.extract_court_from_path(case_id)
        )

        # Extract year information
        result["year"] = (
            self.extract_year_from_case_id(case_id)
            or self.extract_year_from_content(content)
            or quote.get("case_year")
        )
        ...
        result["company"] = self.extract_company_from_content(
            content
        ) or self.extract_company_from_content(quote.get("speaker", ""))

        return result

    def enrich_quotes_with_provenance(
    ...
    def extract_provenance_from_doc(self, doc: Dict[str, any]) -> Dict[str, any]:
        ...
        result["court"] = (
            meta.get("court")
            or self.extract_court_from_doc_id(doc_id)
            or self.extract_court_from_path(source_uri)
        )

        # Extract year from metadata or content
        result["year"] = (
            meta.get("year")
            or self.extract_year_from_content(doc.get("raw_text", ""))
            or self.extract_year_from_case_id(str(meta.get("docket", "")))
        )
        ...
        result["company"] = meta.get("party") or self.extract_company_from_content(
            doc.get("raw_text", "")
        )
...
def add_provenance_to_quotes_batch(
    ...
    with (
        open(quotes_file, "r", encoding="utf-8") as infile,
...
def add_provenance_to_docs_batch(
    ...
    with (
        open(docs_file, "r", encoding="utf-8") as infile,
        open(output_file, "w", encoding="utf-8") as outfile,
    ):
