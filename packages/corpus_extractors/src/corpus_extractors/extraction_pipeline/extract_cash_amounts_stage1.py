...
def apply_csv_labels_to_jsonl(csv_path, jsonl_input_path, jsonl_output_path):
    ...
    applied_count = 0
    with (
        open(jsonl_input_path, "r", encoding="utf-8") as infile,
...
def extract_cash_amounts_stage1(
    input_pattern: str,
    output_file: str,
    ...
    """
    Extract cash amounts from documents (stage 1).

    This is a standalone function that calls the main extraction logic.
    ...
    Args:
        ...
        csv_output: CSV output path (optional)
        max_cases: Maximum number of cases
        apply_csv_labels: CSV labels to apply (optional)
        case_position_threshold: Case position threshold
        docket_position_threshold: Docket position threshold
        disable_spacy: Whether to disable spaCy
        disable_spelled: Whether to disable spelled amounts
        disable_usd: Whether to disable USD amounts
        disable_calcs: Whether to disable calculations
        disable_regex: Whether to disable regex
    """
    main(
        input_pattern,
        output_file,
