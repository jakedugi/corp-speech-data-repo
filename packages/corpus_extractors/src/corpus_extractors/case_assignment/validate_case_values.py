...
def validate_row_count(quotes_in, quotes_out):
    ...
    if len(quotes_in) != len(quotes_out):
        print(
            f"❌ FAIL: Row count mismatch! Input: {len(quotes_in)}, Output: {len(quotes_out)}"
        )
        return False
    ...
    for case_id, case_quotes in quotes_by_case.items():
        ...
        if preferred_outcomes:
            ...
            if f"outcome_metadata.{preferred_outcome_type}" not in actual_source:
                print(
                    f"❌ FAIL: Case {case_id} has preferred outcome but didn't use it. "
        ...
        if voted_cash:
            ...
            if actual_source != "cash_amount.highest_votes":
                print(
                    f"❌ FAIL: Case {case_id} has voted cash but didn't use it. "
                    f"Source: {actual_source}"
                )
                errors += 1
            continue
        ...
        if actual_value != "N/A" or actual_source != "N/A":
            print(
                f"❌ FAIL: Case {case_id} should be N/A but got {actual_value} ({actual_source})"
            )
            errors += 1
...
def validate_case_values(
    quotes_in_path: Path,
    quotes_out_path: Path,
    outcomes_path: Path,
    cash_path: Path,
    preferred_outcome_type: str = "stipulated_judgment",
):
    """Validate case value assignments programmatically."""
    quotes_in = load_jsonl(quotes_in_path)
    quotes_out = load_jsonl(quotes_out_path)
    outcomes = load_jsonl(outcomes_path)
    cash_amounts = load_jsonl(cash_path)
    ...
    all_pass &= validate_priority_logic(
        quotes_out, outcomes, cash_amounts, preferred_outcome_type
    )
...
def main():
    ...
    parser.add_argument(
        "--quotes_in", type=Path, required=True, help="Original quotes file"
    )
    parser.add_argument(
        "--quotes_out", type=Path, required=True, help="Output quotes file"
    )
    ...
    all_pass &= validate_case_consistency(quotes_out)
    all_pass &= validate_priority_logic(
        quotes_out, outcomes, cash_amounts, args.preferred_outcome
    )
