...
The corpus_extractors module serves as the information extraction layer for the corpus pipeline, providing:
...
Input should be normalized JSONL documents from `corpus_cleaner`:
...
**Note**: Types are defined in `corpus_types` module following SSOT principles.
...
The corpus_extractors module integrates with:
- **corpus_cleaner**: Consumes normalized documents (input)
- **corpus_features**: Provides quotes for feature extraction (output)
- **corpus_types**: Uses document and quote schemas (SSOT for all types)
