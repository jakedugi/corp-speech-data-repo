"""
JSON Schema export utilities for corpus_types.
...
from corpus_types.schemas.models import (  # Wikipedia Key People models
    APIConfig,
    CasePrediction,
    CaseVector,
    DatasetManifest,
...
def export_model_schema(
    ...
    Returns:
        JSON schema dictionary with corpus_types metadata
    """
    schema = model_cls.schema()

    # Add corpus_types specific metadata
    schema.update(
        {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "$id": f"https://corpus_types.example.com/schemas/{model_cls.__name__.lower()}.json",
...
def export_all_schemas(output_dir: Path, version: str = "1.0.0") -> None:
    """
    Export all corpus_types schemas to JSON files.
