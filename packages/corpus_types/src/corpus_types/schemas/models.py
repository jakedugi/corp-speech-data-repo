"""
Pydantic models for corpus_types data contracts.
...
class AdapterProv(StrictBase):
    ...
    name: Literal["corpus_hydrator"] = Field(..., description="Adapter name")
...
class CourtListenerConfig(APIConfig):
    ...
    default_page_size: int = Field(default=50, description="Default results per page")
    ...
    max_chunk_size: int = Field(default=50, description="Maximum company chunk size")

    # Async settings
    max_concurrency: int = Field(default=2, description="Maximum concurrent requests")
    ...
    statute_queries: Dict[str, str] = Field(
        default_factory=dict, description="Statute-specific search query templates"
    )
...
class Quote(ExtensibleBase):
    ...
    court: Optional[str] = Field(
        None, description="Court name (extracted from doc_id or metadata)"
    )
    law: Optional[str] = Field(
        None, description="Law type (extracted from query or content)"
    )
    company: Optional[str] = Field(
        None, description="Company name (extracted from search or headers)"
    )
...
class CashAmountCandidate(StrictBase):
    ...
    feature_votes: int = Field(
        ..., description="Number of extraction features that matched"
    )
...
class IndexConstituent(StrictBase):
    ...
    date_added: Optional[str] = Field(
        None, description="Date company was added to index"
    )
...
from .wikipedia_key_people import __version__ as wikipedia_key_people_version
from .wikipedia_key_people import (
    get_default_config,
