...
class IndexConfig(BaseModel):
    ...
    cik_column: Optional[int] = Field(
        default=6, description="Column index for CIK numbers"
    )
...
class ScrapingConfig(BaseModel):
    ...
    max_people_per_title: int = Field(
        default=5, description="Maximum people per title type"
    )
    max_companies: Optional[int] = Field(
    ...
    # HTTP pool settings
    wikipedia_pool_connections: int = Field(
        default=50, description="Wikipedia connection pool size"
    )
    wikipedia_pool_maxsize: int = Field(
        default=50, description="Wikipedia max pool size"
    )
    ...
    wide_format_filename: str = Field(
        default="{index}_aliases_enriched.csv",
        description="Wide format output filename",
    )
    long_format_filename: str = Field(
        default="{index}_officers_cleaned.csv",
        description="Long format output filename",
    )
    base_list_filename: str = Field(
        default="{index}_aliases.csv", description="Base company list filename"
    )
...
class ValidationConfig(BaseModel):
    ...
    cik_pattern: str = Field(
        default=r"^\d{10}$", description="CIK validation regex pattern"
    )
    ...
    ticker_pattern: str = Field(
        default=r"^[A-Z]{1,5}$", description="Ticker validation regex pattern"
    )

    # Data quality checks
    min_officers_required: int = Field(
        default=1, description="Minimum officers required per company"
    )
    max_officers_per_company: int = Field(
        default=50, description="Maximum officers per company"
    )
    require_unique_officers: bool = Field(
        default=True, description="Require unique officer names per company"
    )
...
def validate_config(config: WikipediaScraperConfig) -> List[str]:
    ...
    if (
        config.validation.max_officers_per_company
        < config.validation.min_officers_required
    ):
        issues.append(
            "Max officers per company cannot be less than min officers required"
        )
