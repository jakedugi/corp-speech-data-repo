...
class TestAdapterProv:
    ...
    def test_valid_adapter_prov(self):
        ...
        adapter = AdapterProv(
            name="corpus_hydrator", version="1.2.3", git_sha="abc123def456"
        )
        assert adapter.name == "corpus_hydrator"
    ...
    def test_adapter_prov_required_fields(self):
        """Test that required fields are enforced."""
        with pytest.raises(ValidationError):
            AdapterProv(name="corpus_hydrator")  # missing version
    ...
    def test_adapter_prov_optional_fields(self):
        ...
        adapter = AdapterProv(name="corpus_hydrator", version="1.2.3")
...
class TestProducer:
    ...
    def test_producer_required_fields(self):
        """Test that required fields are enforced."""
        with pytest.raises(ValidationError):
            Producer(name="corpus_extractors")  # missing version
    ...
    def test_producer_name_validation(self):
        ...
        # Valid names
        Producer(name="corpus_cleaner", version="1.0.0")
        Producer(name="corpus_extractors", version="1.0.0")
