...
class QuoteExtractor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("QuoteExtractor initialized with comprehensive S&P 500 corporate data")
    
    def _load_default_config(self) -> Dict[str, Any]:
        """Load default configuration from quotes_comprehensive.py or fallback to quotes.yaml."""
        # Try comprehensive config first
        comprehensive_config_path = Path(__file__).parent.parent.parent / "configs" / "quotes_comprehensive.py"
        if comprehensive_config_path.exists():
            try:
                ...
                logger.info(f"Loaded comprehensive config from {comprehensive_config_path}")
        ...
        if HAS_YAML:
            yaml_config_path = Path(__file__).parent.parent.parent / "configs" / "quotes.yaml"
            try:
        ...
        logger.warning(f"No config files found or YAML not available, using minimal defaults")
