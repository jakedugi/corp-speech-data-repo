...
def get_nlp(model_name: str = "en_core_web_sm", enable_ner: bool = True, use_gpu: bool = False):
    """
    ...
    try:
        ...
        if enable_ner:
            ...
            if test_doc.ents:
                logger.info(f"NER is working - found {len(test_doc.ents)} entities in test")
            else:
        ...
        logger.warning(f"Could not load {model_name}: {e}, using basic English pipeline")
...
class Attributor:
    ...
    def __init__(self, company_aliases: Set[str], spacy_model: str = "en_core_web_sm",
                 role_keywords: List[str] = None, executive_names: List[str] = None,
                 enable_ner: bool = True, use_gpu: bool = False):
        """
