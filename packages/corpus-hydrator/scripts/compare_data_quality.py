...
def compare_data_quality():
    ...
    for ticker in sample_tickers:
        if ticker in original_data and ticker in enhanced_data:
            ...
            if enh.get('business_city'):
                print(f"   Business Address: {enh['business_city']}, {enh.get('business_state', '')}")
    ...
    print(f"   Companies with SEC entity names: {enhanced_count} ({enhanced_count/len(original_data)*100:.1f}%)")
    print(f"   Companies with business addresses: {address_count} ({address_count/len(original_data)*100:.1f}%)")
