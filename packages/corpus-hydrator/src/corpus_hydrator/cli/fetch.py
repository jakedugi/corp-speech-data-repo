...
def wikipedia(
    index: str = typer.Option(
        "sp500", "--index", help="Market index to scrape (sp500, dow, nasdaq100, russell1000)"
    ),
    output_dir: Path = typer.Option(
...
def index_constituents(
    ...
    force: bool = typer.Option(
        False, "--force", help="Force refresh (ignore cache)"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Enable verbose logging"
    ),
        ...
        if index == "all":
            ...
            if result.success and result.constituents:
                ...
                if index == "sp500":
                    ...
                    print("sp500 = sp500[[\"Symbol\", \"Security\", \"GICS Sector\", \"Date first added\"]]")
                    print("print(sp500.head())")

                    # Filter to legacy format for display
                    display_df = df[['symbol', 'company_name', 'sector', 'date_added']].copy()
                    display_df.columns = ['Symbol', 'Security', 'GICS Sector', 'Date first added']
