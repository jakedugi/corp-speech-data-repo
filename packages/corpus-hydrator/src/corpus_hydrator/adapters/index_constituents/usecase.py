...
from .parsers.base import TableParser, ParserError
...
class IndexExtractionUseCase:
    ...
    def execute(self, index_key: str) -> IndexExtractionResult:
        ...
        try:
            ...
            logger.info(f"Successfully extracted {len(constituents)} constituents for {config.name}")
            return IndexExtractionResult(
                index_name=config.name,
...
def extract_index(index_key: str, provider: IndexProvider, parser: TableParser) -> IndexExtractionResult:
    """
